...
vcom -2008 -work work -quiet $DUT/DHU-project/simulation/tb/router_controller_top_tb/system_3SHyLoC_tb.vhd
vsim -coverage work.system_3SHyLoC_tb -voptargs="+acc"
# vsim -coverage work.system_3SHyLoC_tb -vopt -t 1ns -voptargs="+acc"
#wave file
do $DUT/DHU-project/simulation/sim_signal/system_3SHyLoC_addwave.md 
run -all
# Create the coverage report directory
set coverage_dir [file dirname "$DUT/DHU-project/simulation/coverage/system_3SHyLoC_tb.covhtml"]
file mkdir -p $coverage_dir
coverage report -output $DUT/DHU-project/simulation/coverage/system_3SHyLoC_tb.covhtml -html -cvg -details -assert -codeAll
