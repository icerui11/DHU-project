...
vcom -2008 -work work -quiet $DUT/DHU-project/simulation/tb/system_SHyLoC_top_v2_tb.vhd
vsim -coverage work.system_SHyLoC_top_v2_tb -vopt -t 1ns -voptargs="+acc"
#wave file
do $DUT/DHU-project/simulation/sim_signal/system_top_signalwave.md 
run -all
# Create the coverage report directory
set coverage_dir [file dirname "$DUT/DHU-project/simulation/coverage/system_SHyLoC_top_v2_tb.covhtml"]
file mkdir -p $coverage_dir
coverage report -output $DUT/DHU-project/simulation/coverage/system_SHyLoC_top_v2_tb.covhtml -html -cvg -details -assert -codeAll
