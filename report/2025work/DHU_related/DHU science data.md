...
An observation is a set of acquisitions having a similar (or at least linked) context and set of parameters. It is related to a sequence of operations and actions performed at instrument level to obtain a consistent set of data. This sequence of operations and actions performed at instrument level to obtain a consistent set of data.


Frame： A frame is the raw result produced by the detector for a single integration. It is a matrix of electron counts for each pixel. It is a matrix of electron counts for each pixel (in ADU).
